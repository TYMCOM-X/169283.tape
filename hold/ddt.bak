...
PROMPT	BC	3
	DA	'*"0D"0A'	:STRING LENGTH 1  STRING "*CRLF"
TERR	BC	11
...
XDDT	ST	LINK,SLINK,BASE	:SAVE LINK
	LH	R0,FLAG		:CHECK IF USER TYPED BREAK CHAR
	JEFS	XDDTO		:HE DID NOT - CHECK IF BP OR ILL INST
	LIS	R0,0		:CLEAR FLAG
	STH	R0,FLAG
	LHI	NLF,-2		:REMEMBER USER PROG STOPPED FOR BREAK CHAR
	CLHI	SB,0		:IF SPECIAL BP'S SET - REMOVE THEM
	JER	LINK		:NO SPCIAL BP'S, RETURN
	LIS	SB,0		:CLEAR SPECIAL BP FLAG
	LIS	N,8		:CLEAR FIRST SPECIAL BP
	JAL	LINK,SBB
	LIS	N,9		:CLEAR SECOND SPECIAL BP
	JAL	LINK,SBB
	J	LX4		:RESTORE LINK AND RETURN
XDDTO	L	X,OLDPSW+4,BASE	:GET ADDR OF ILL INST
...
P	CLHI	NLB,-2		:DID USER STOP DUE TO BREAK CHAR
	JE	YY,BASE		:YES - RETURN TO USER CODE
	CLHI	NLB,-1		:NLB = -1  IS ERROR
	JE	PERR
...
ILOOP	ST	SB,LISTA-4,BASE,R1 :ZERO THIS ENTRY
	...
	JNBS	ILOOP		:LOOP
	JR	LINK		:RETURN
 : EXIT INSTRUCTION - RETURN TO COMMAND HANDLER - DDT INST E
E	SVC	EXIT,0
