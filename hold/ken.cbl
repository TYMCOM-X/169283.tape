...
FILE-CONTROL.
        ...
        SELECT SHIPDED-DAT ASSIGN TO DSK
        RECORDING MODE IS ASCII.
...
FD SHIPED-DAT
        ...
        DATA RECORDS ARE SHIP-REC-IN.
01 SHIP-REC-IN                         PIC X(201).

FD SHIPED-NEW
        LABEL RECORDS ARE STANDARD
        VALUE OF IDENTIFICATION IS "SHIPEDNEW"
        DATA RECORDS ARE SHIP-REC-OUT.
01 SHIP-REC-OUT.
       03 S-DATE                       PIC X(6).
       03 S-KEY                        PIC X(7).
...
FORK.
       IF T-SORT-KEY = HIGH-VALUES  GO TOFINISH-MASTER.
...
CHECK-MAST-SEQ.
        IF MAST-SEQ-CHECK  LESS THAN LAST-SEQ-CHECK
            ...
            STOP RUN.
        IF MAST-SEQ-CHECK  EQUAL TO LAST-SEQ-CHECK AMAST-REC-TYPE NOT = 'N'
...
LOAD-NOTE-OR-MODEL.
       PERFORM READ-MASTER-REC  THRU CHECK-MAST-SEQ.
       IF MAST-FORK-KEY NOT EQUAL LAST-FORK-KEYNUMB OR
...
GET-NEXT-TRANS.
       ...
       IF NOTE (T-NEW-ENTRY OR T-ADDITION)
           DISPLAY "ACTIVITY NOT PERMITTED ON NEW ORDERS."
           GO TO REJECT-TRANS.
...
CHECK-FOR-DELETED.
       IF A-DELETE-FLAG = 99 OR IT-DELTE-FLAG = 99 GO TO FORK.

CHECK-FOR-SHIPMENT.
       SEARCH M-REC  AT END GO TO CHECK-FOR-WORK-ORDER
           WHEN M-SERIAL-NUMB NOT EQUAL SPACES  NEXT SENTENCE.
...
LOOP-THRU-MODELS.
       ...
       IF M-TYPE (MODEL-INDX) = 'P'
           MOVE TODAYS-DATE TO M-SHIP-DATE (MODEL-INDX)
           GO TO LOOP-THRU-MODELS.
       IF M-SERIAL-NUMB (MODEL-INDX) EQUAL SPACES
           AND M-SHIP-DATE (MMDEL-INDX) EQUAL SPACES
           DISPLAY "WARNING -- PARTIAL SHIPMENT FOR: ", M-KEY (MODEL-INNDX)
           DISPLAY "           EXCLUDES MODEL NUMBER: ", M-NUMB (MODEL-INDX)
           GO TO LOOP-THRU-MODELS.

WRITE-SHIPED-NEW.
       IF M-SHIP-DATE (MODEL-INDX) = SPACES  MOVE TODAYS-DATE TO M-SHIP-DATE (MODEL-INDX).
       SEARCH PRICE-TABLE  AT END GO TO LOGIC-ERROR
           WEHN M-NUMB (MODEL-INDX) = P-MODEL-NUMB (PRICE-INDX) NEXT SENTENCE.
...
CHECK-FOR-ALL.
       SEARCH M-REC   AT TEND GO TO FORK
