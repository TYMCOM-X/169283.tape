TITLE ALPHA% %1.(100) 	ALPHANUMBERIC INPUT/OUTPUT ROUTINES
	SUBTTL	D. TODD /DRT/     08-DEC-1973
...
;	DEFINE THE LOADING PARAMETERS
	...
	ENTRY	ALPHA%
	EXTERN	IBYTE.,OBYTE.,WPOINT
ALPHA%:		;GENERAL ENTRY POINT
	MOVEI	T5,5		;ASSUME SINGLE PRECISION
	MOVE	T1,DAT.TP+2(P4)	;GET THE VARIABLE TYPE
	CAIN	T1,TP%DOR	;DOUBLE PRECISION REAL
	LSH	T5,1		;YES, ALLOW 10 CHARACTERS MAX.
	LDB	T4,WPOINT	;GET THE WIDTH FIELD
	JUMPE	T4,ALPHI2	;EXIT W=0
	MOVSI	T2,(POINT 7,(G1));SET UP AN ASCII BYTE POINTER
	MOVEI	T3,(T4)		;SAVE THE W FIELD
	SUBI	T3,(T5)		;COMPUTE THE EXCESS FIELD SIZE
	CAILE	T4,(T5)		;IS THE W FIELD IN RANGE
	MOVEI	T4,(T5)		;NO, SET TO MAX SING.=5 DP=10
...
ALPHI:	MOVE	T0,[ASCII /     /];GET A SET OF 5 BLANKS
	...
	JUMPLE	T3,ALPHI1	;NO EXCESS W<= MAX FIELD SIZE
	JSP	P1,IBYTE.	;EXCESS W SKIP INPUT CHARACTERS
	SOJG	T3,.-1		;CONTINUE SKIPPING
ALPHI1:	JSP	P1,IBYTE.	;GET AN INPUT BYTE
	IDPB	T0,T2		;PUT IN USER'S VARIBLE
	SOJG	T4,ALPHI1	;CONTINUE UNTIL W=0
ALPHI2:	POPJ	P,		;RETURN FOR FOROTS

ALPHO:				;ALPHA OUTPUT ROUTINE
	JUMPLE	T3,ALPHO1	;IS OUTPUT FILL NEEDED
...
ALPHO1:	ILDB	T0,T2		;GET THE CHARACTER FORM THE VARIABLE
	...
	SOJG	T4,ALPHO1	;CONTINUE UNTIL W=0
	POPJ	P,		;RETURN TO FOROTS
	PRGEND
TITLE FLIRT%	%1(100)	FLOATING POINT INPUT
	SUBTTL	D. TODD /DRT/     08-DEC-1973	D.M.NIXON
;***COPYRIGHT 1972,1973 DIGITAL EQUIPMENT CORP., MAYNARD, MASS.***
;FROM %32 REWRITE BY		DAVE NIXON, TOM EGGERS
...
;	PUSH	P,G3		;DON'T NEED G3
	...
	LDB	W,WPOINT	;GET THE FIELD WIDTH
...
ERROR:	SKIPN	ILLEG.(P4)	;ILLEGAL CHAR. FLAG SET?
...
ADDCNT:
ENDF:
ENDF1:	MOVE	G4,(P)		;GET THE WIDTH FIELD BACK
	TRNE	F,DOTFL		;HAS DECIMAL POINT BEEN INPUT?
	JRST	ENDF2		;YES
	LDB	D,DPOINT	;NO, GET DIGITS AFTER POINT FROM FORMAT
...
IFE CPU-KA10,<
	...
	PRGEND
TITLE FLOUT%	%1(100)  	FLOATING POINT OUTPUT	FORTRAN IV
	SUBTTL	D. TODD /DRT/     08-DEC-1973	D.M.NIXON/DRT
;***COPYRIGHT 1972,1973 DIGITAL EQUIPMENT CORP., MAYNARD, MASS.***
;FROM	V%32 REWRITE BY		D. M. NIXON & T. W. EGGERS
...
RETRY:	MOVE	T0,(G1)		;LOAD AC 0 WITH NUMBER
	JUMPSP	.+2		;DOUBLE PRECISION?
	MOVE	T1,1(G1)	;YES, GET LOW WORD ALSO
...
FLOUT6:	G4,(P)		;GET THE FIELD PARAMETERS
	LDB	W,WPOINT
	LDB	D,DPOINT
...
TRYFIT:	JUMPGE	C,FIT		;WILL FIT FORMAT
	...
	TLNN	P2,FT.ETP!FT.PRC	;IF E FORMAT
...
PRGEND
TITLE INTEG%  %1.(100)  DECIMAL INTEGER INPUT/OUTPUT FORTRAN IV
	SUBTTL	D. TODD /DRT/     08-DEC-1973
...
INTEG%:			;INTEGER INPUT/OUTPUT CONVERSION ROUTINE
	LDB	T3,WPOINT	;GET THE FIELD WIDTH
	SETZB	T5,T2		;CLEAR STORAGE
	JUMPO	INTO		;GO TO INTO (OUTPUT)
	JUMPN	T3,INTI1	;W=0 VARIABLE FIELD
...
INTI4:	JUMPL	T3,INTI6	;NO, CHECK FOR VARIABLE FIELD
...
IFE ASTFL,<	ADD	T2,T4	;GET THE NUMBER OF DIGITS
...
INTO6:	POPJ	P,		;RETURN TO FOROTS
	PRGEND
TITLE	LOGIC% %1.(100) LOGICAL INPUT/OUTPUT CONVERSION ROUTINES
	SUBTTL	D. TODD /DRT/     08-DEC-1973
;***COPYRIGHT 1972,1973 DIGITAL EQUIPMENT CORP., MAYNARD, MASS.***
...
LINT1:	SOJL	T4,LINT3	;SPACING REQUIRED W>=0
	JSP	P1,IBYTE.	;YES, SKIP AN INPUT CHARACTER
	JRST	LINT1		;CONTINUE UNTIL W<0
...
LOUT:				;LOGICAL VARIABLE OUTPUT ROUTINE
	...
	TLNN	P2,FT.GTP	;CHECK FOR G CONVERSION TYPE
	JRST	LOUT3		;NO, W=0 EXIT
	MOVEI	T4,16		;G TYPE DEFAULT L15
...
LOUT3:	POPJ	P,		;RETURN TO FOROTS
	PRGEND
TITLE OCTAL% %1.(100) OCTAL INPUT/OUTPUT CONVERSION ROUTINE
	SUBTTL	D. TODD /DRT/     08-DEC-1973
;***COPYRIGHT 1972,1973 DIGITAL EQUIPMENT CORP., MAYNARD, MASS.***
...
OCTI:				;OCTAL INPUT ROUTINE
	SKIPN	T3		;CKECK FOR VARIABLE INPUT FILD
	SETO	T3,		;YES, SET VARIABLE FLAG
...
OCTI1B:	CAIG	T0,"7"		;CKECK FOR AN OCTAL
	...
	JRST	T3,OCTI2		;56;NO, NOT AN OCTAL DIGIT
	TLO	T2,400000	;SET DIGIT SEEN FLAG
...
OCTI3:	JUMPL	T3,OCTI5	;NO ERROR ON VARIABLE FIELD INPUT
...
OCTO3:	POPJ	P,		;YES, RETURN TO FOROTS
	PRGEND
TITLE	POWTB%	%1.(100)	D.P. INTEGER POWER OF TEN TABLE
	SUBTTL	D. TODD /DRT/     08-DEC-1973	TOM EGGERS
;***COPYRIGHT 1972,1973 DIGITAL EQUIPMENT CORP., MAYNARD, MASS.***
...
>	;END OF REPEAT 0
	...
	PRGEND
	TITLE LSTDR% V2.(112) LIST DIRECT I/O MODULE
	SUBTTL	D. TODD /DRT/	24-JAN-1973
...
LSTDR%:
	...
	MOVSI	P2,FT.LSD	;SET THE NO FILL FLAG FOR LIST DR.
...
LSTDR1:			;LIST DIRECTED DISPATCH ROUTINE INTO NMLST%
	...
	TLZ	P2,FT.ELT!FT.SLT;KILL THE EXTENDED LIST FLAGS
	HRRZ	T1,NMLTBL##(T5)	;GET THE NAME/LIST INPUT  DISPATCH
	JUMPI	LSTDR2		;JUMP ON INPUT
...
LSTDR2:	PUSHJ	P,(T1)		;OUTPUT THE VARIABLE
	...
	PRGEND
TITLE	NMLST%	%1.(107)	NAMELIST I/O
	SUBTTL	D. TODD /DRT/     08-DEC-1973
;***COPYRIGHT 1972,1973 DIGITAL EQUIPMENT CORP., MAYNARD, MASS.***
...
NMLST%:
	MOVEI	P2,(G1)		;PUT THE NAME LIST POINTER IN A P AC
...
NLOVAR:	ADDI	P2,1		;MOVE POINTER TO NEXT VARIABLE
	SKIPN	(P2)		;ARE WE AT THE END OF THE TABLE?
...
NLOFIN:	MOVEI	T0,"$"		;GET A DOLLAR SIGN TO TERMINATE
	...
	PJRST	NXTLN.		;FINISH UP LINE (NEVER RETURN)
...
NLIN1:	PUSHJ	P, NXTLN.	;CALL FOROTS. FOR THE NEXT RECORD
	JRST	NLIN.		;TRY AGAIN
...
NLIAR5:	PUSHJ	P,NLIEQU	;SCAN FOR AN EQUAL SIGN
	...
	CAIN	T5,TP%DOR	;UNLESS THE VARIABLE IS 
	TLO	P2,FT.PRC	;DOUBLE PRECISION
...
NLII:	PUSHJ	P,NLISCN	;FIND SOME DATA
	PUSHJ	P,INTEG%	;GET AN INTEGER VARIABLE
	JSP	P1,NLIDLM	;CHECK THE DELIMITER
	AOBJN	G1,NLII		;CONTINUE
	POPJ	P,		;RETURN
...
NLIF:	PUSHJ	P,NLISCN	;FIND SOME DATA
	...
	JSP	P1,NLIDLM	;CHECK THE DELIMITER
	JUMPGE	P2,NLID1	;JUMP ON SINGLE PRECISION
...
NLIC1:			;REPEAT COUNT FOR COMPLEX
	...
	DMOVE	T0,-1(G1)	;GET THE COMPLEX NUMBER BACK
...
NLIS:				;INPUT A STRING OF ASCII TEXT
	...
	MOVSI	T1,(POINT 7,(G1));ASCII BYTE POINTER
...
NLIS1:	JSP	P1,IBYTE.##	;GET A CHARACTER
	...
	AOBJN	G1,NLIS		;END OF ARRAY
	TLNN	P2,FT.LSD	;LIST DIRECTED I/O
	POPJ	P,		;NO, NAMELIST EXIT
	JSP	P1,IOLS%%##	;LIST DIRECTED GET NEXT VARIABLE
	JRST	NLIS		;CONTINUE WITH NEW VARIABLE
...
NLISCN:	JSP	P1,IPEEK.	;LOOK AT THE NEXT CHARACTER
...
IFE CPU-KI10,<FIX	G2,G2	;CONVET TO INTEGER>
IFE CPU-KA10,<
...
NLIDL1:	SOJLE	G2,(P1)		;DONE WITH REPEAT
	...
	JUMPL	P2,NLIDL2	;JUMP ON DOUBLE PRECISION
	MOVEM	T0,(G1)		;STORE THE SINGLE PRECISION RESULT
	JRST	NLIDL1		;TRY AGAIN
NLIDL2:	DMOVEM	T0,1(G1)	;STORE THE DOUBLE PRECISION RESULT
	ADD	G1,[XWD 1,1]	;STEP
