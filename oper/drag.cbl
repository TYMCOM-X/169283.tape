...
001020 PROGRAM-ID.  'SALESUMB'.                                         SALESUMB
...
001080     SELECT INVOICE-CARD-FILE, ASSIGN TO 'SYSIN' UTILITY.         SALESUMB
001090     SELECT CURR-TRANS-FILE, ASSIGN TO 'CTFILE' UTILITY.          SALESUMB
001100     SELECT REPT-BAL-OUT,    ASSIGN TO 'OUTPUT' UTILITY.          SALESUMB
001110     SELECT DIST-ID-FILE ASSIGN TO 'DISTID' UTILITY.              SALESUMB
002010 DATA DIVISION.                                                   SALESUMB
002020 FILE SECTION.                                                    SALESUMB
002030 FD  INVOICE-CARD-FILE                                            SALESUMB
002040         RECORDING MODE F, LABEL RECORDS OMITTED, RECORD CONTAINS SALESUMB
002050           80 CHARACTERS, DATA RECORD IS CURRENT-INVOICE.         SALESUMB
...
004010 FD  CURR-TRANS-FILE                                              SALESUMB
004020       RECORDING MODE F, BLOCK CONTAINS 10 RECORDS, RECORD        SALESUMB
004030         CONTAINS 60 CHARACTERS, LABEL RECORDS OMITTED,           SALESUMB
004035           DATA RECORD IS C-T-REC.                                SALESUMB
...
005010 FD  REPT-BAL-OUT                                                 SALESUMB
005020       RECORDING MODE F, LABEL RECORDS OMITTED, RECORD CONTAINS   SALESUMB
005030         133 CHARACTERS, DATA RECORDS ARE BAL-TRANS-OUT,          SALESUMB
005035           INV-TOTALS-OUT.                                        SALESUMB
...
006110       10 FILLER             PICTURE X(4).                        SALESUMB
...
006150     05 FILLER               PICTURE X(3).                        SALESUMB
...
006210     05 FILLER               PICTURE X(29).                       SALESUMB
...
006260     05 FILLER               PICTURE X.                           SALESUMB
006270 FD  DIST-ID-FILE                                                 SALESUMB
006280       RECORDING MODE F, LABEL RECORDS OMITTED, BLOCK CONTAINS    SALESUMB
006290         10 RECORDS, RECORD CONTAINS 60 CHARACTERS,               SALESUMB
006300           DATA RECORD IS DIST-ID-ENTRY.                          SALESUMB
...
007050   77 SKIP-TO-NEXT-PAGE      PICTURE X      VALUE '1'.            SALESUMB
...
008130       05 FILLER             PICTURE X(17)  VALUE SPACES.         SALESUMB
...
010580     OPEN INPUT DIST-ID-FILE.                                     SALESUMB
010590       MOVE ZERO TO I.                                            SALESUMB
010600   LOAD-ID-ARRAY.                                                 SALESUMB
010610     READ DIST-ID-FILE, AT END GO TO CLOSE-DIST-ID-FILE.          SALESUMB
010620         ADD 1 TO I.                                              SALESUMB
010630         MOVE DIST-NO-ID TO D-NO (I).                             SALESUMB
010640         MOVE DIST-NAME TO D-NAM (I).                             SALESUMB
010650     IF  ITEM-3 GREATER THAN 99990, GO TO CLOSE-DIST-ID-FILE.     SALESUMB
...
010700              'ERROR    DIST-ID-FILE GT 75 ENTRIES',              SALESUMB
010710              PERFORM CLOSE-DIST-ID-FILE, STOP RUN.               SALESUMB
010720   CLOSE-DIST-ID-FILE.                                            SALESUMB
010730     CLOSE DIST-ID-FILE.                                          SALESUMB
011020   PREP-FOR-REPORT-BAL.                                           SALESUMB
011030     OPEN INPUT INVOICE-CARD-FILE, OUTPUT CURR-TRANS-FILE,        SALESUMB
011040          REPT-BAL-OUT.                                           SALESUMB
011050     READ INVOICE-CARD-FILE, AT END GO TO QUIT.                   SALESUMB
...
01    MOVE SPACES TO BAL-TRANS-OUT.                                SALESUMB
...
011190     READ INVOICE-CARD-FILE, AT END GO TO QUIT.                   SALESUMB
...
011260     IF D-NO (I) NOT GREATER THAN DIST-NO IN CURR-INVOICE,        SALESUMB
...
012070     COMPUTE EXT-GROSS IN C-T-REC ROUNDED = QTY IN C-T-REC *      SALESUMB
...
013050     ADD GROSS-CST-INV-TOT TO GROSS-CST-GRAND-TOT, ROUNDED.       SALESUMB
013060     ADD NET-SALE-INV-TOT TO NET-SALE-GRAND-TOT, ROUNDED.         SALESUMB
...
013170     ADD EXT-GROSS IN C-T-REC TO GROSS-CST-INV-TOT, ROUNDED.      SALESUMB
013180     ADD QTY IN C-T-REC TO QTY-INV-TOT.                           SALESUMB
013190     ADD TOT-NET-SALE-1 TO NET-SALE-INV-TOT, ROUNDED.             SALESUMB
...
015020     READ INVOICE-CARD-FILE, AT END PERFORM SUB-TOTALS-OUT THRU   SALESUMB
...
015130     CLOSE REPT-BAL-OUT, CURR-TRANS-FILE, INVOICE-CARD-FILE.      SALESUMB
