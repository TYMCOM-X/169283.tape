...
3102 Q1=W1*(Y2-1)+INT((Q2-1)/3)*3+3
3105 MAT M=ZER
3110 MAT P= ZER
3115 MAT T = ZER
3120 MAT N= ZER
...
3785 PRINT USING 379
3790 :           INCOME  TION PLUS   ON AMORT  PRINCIPAL   EXPENSES  CASH FLOW  OR GAIN    OR PAYMENT CASH FLOW  AFTER TAX '
...
8102 K4=INT(K4+.5)
8104 K5=INT(K5+.5)
8105 T=C(Y,2)+K4+K5-N(Y,6)-N(Y,7)
8106 C(Y,1)=INT(C(Y,1)+.5)
8107 C(Y,2)=INT(C(Y,2)+.5)
8108 N(Y,6)=INT(N(Y,6)+.5)
8109 N(Y,7)=INT(N(Y,7)+.5)
8110 X=(T-C(Y,1)*Z1)/(1-Z1)
8111 IF FNM(Y,12)<>0 THEN 8120
8115 PRINT Y;INT(X+.5);C(Y,1);C(Y,2);N(Y,6);N(Y,7);K4;K5
8120 NEXT Y
9000 GOTO 9992
9990 GOSUB 8000
9992 END
