...
100 N$=""
101 DIM A(360,3),B(440,20),K(2,25),H(2,55),T(400),M(400,10),N(400,7),E(110)
102 PRINT"DATA FILE";
103 INPUT K$
104 FILE #1,K$
105 PRINT "REPORT WIDTH: NARROW OR WIDE ";
110 INPUT W$
111 W3=2
112 IF LEFT$(W$,1)="W" THEN 115
113 W3=1
115 READ #1,L$
116 READ #1,W$
117 IF LEFT$(W$,1)="M" THEN 121
118 IF LEFT$(W$,1)="Q" THEN 124
119 IF LEFT$(W$,1)="S" THEN 127
120 IF LEFT$(W$,1)="A" THEN 130
121 O$="MONTH"
122 W1=12
123 GOTO 134
124 O$="QUART"
...
152 D0=3
154 S$="N"
156 IF W1<4 GO TO 200
158 PRINT"CONVENTIONS: HALF-YEARLY=1; MODIFIED HALF YRLY=2; AND REGULAR =3 ";
160 INPUT D0
162 IF D0=3 GOTO 200
164 PRINT "NEW CORPORATION ";
166 INPUT S$
168 IF LEFT$(S$,1)<>"Y" THEN 200
170 PRINT O$;" THE CORPORATION CAME INTO EXISTANCE";
172 INPUT N0
200 IF W3>1 GOTO 2680
201 O=100
202 G=.5
203 REM FINAL YR,PRD
204 READ #1,Y2,Q2
205 Q1=W1*(Y2-1)+Q2
210 MAT M=ZER
215 MAT P= ZER
...
312 PRINT" DEBT TYPE SPECIFIED IS ",A$
...
535 REM D/S
540 READ #1,I$
545 READ #1,D1,Y2,Q2,Y3,Q3
550 D2=W1*(Y2-1)+Q2
555 D3=W1*(Y3-1)+Q3
560 FOR Y=D2 TO D3
565 M(Y,4)=D1
570 NEXT Y
575 IF D3>=Q1 GOTO 585
580 GOTO 545
585 B1=0
590 REM "RENT STARTING YR,PRD;ENDING YR,PRD
595 READ #1,I$
600 READ #1, B1,Y2,Q2,Y3,Q3
605 B2=W1*(Y2-1)+Q2
610 B3=W1*(Y3-1)+Q3
615 FOR Y=B2 TO B3
620 M(Y,1)=B1
625 NEXT Y
630 IF B3>=Q1 GOTO 640
635 GOTO 600
640 REM "EXP";
645 READ #1,I$
...
751 INPUT W$
752 M$="A"
753 IF LEFT$(O$,1)="A" THEN 780
756 IF LEFT$(W$,1)="N" THEN 780
757 PRINT "ANNUAL OR ";O$;"LY";
758 INPUT M$
760  
765 IF LEFT$(M$,1)="A" THEN 780
770 M$="P"
...
822 PRINT SPACE$(40);"               100'S OF DOLLARS"
825 FOR I=1TO3
830 PRINT
835 NEXT I
840 PRINT USING 845,N$
845 :PRD  RENTAL  DEP+  INT.   INT. & OTHER INTNL  TAX LS TX SAV AFT TX CUM'
850 PRINT USING 855,N$
855 :     INCOME FT END ON AMR PRINCP EXPEN CASHFL /GAIN  OR PMT CASHFL AFTX'
860 PRINT USING 865, Z1*100
865 :            FEES   DEBT   ON DBT SES   BEF.TX       @###.#%       CASHF
870 PRINT
875 PRINT
...
1155 PRINT
1160 PRINT
1165 PRINT USING 1110, "SALE",K1/O+G,X0/O+G,0,0,0,K1/O+G,(K1-X0)/O+G,(-K2)/O+G,K3/O+G,(T5+K3)/O+G
1170 PRINT
1175 PRINT USING 1180,V1/O+G,V2/O+G,V3/O+G,V4/O+G,V5/O+G,V6/O+G,V7/O+G,V8/O+G,(T5+K3)/O+G,(T5+K3)/O+G
1180:TOT  ###### ###### ##### ###### ##### ###### ###### ###### ###### ###### 
1185 PRINT
1190 B1=Q1+1
...
1566 U$="A"
1567 IFLEFT$(O$,1)="A" THEN 1580
1570 PRINT "ANNUAL OR ";O$;"LY";
1575 INPUT U$
1576 PRINT" "
1577 PRINT" "
1578 PRINT" "
1580 U$=LEFT$(U$,1)
1581 IF U$="A" THEN 1585
1582 U$="P"
1585  PRINT
1590 R6=R*100
...
1620 R7=R5*100
1625 :'L          ----CASH FLOW FROM LEASE----              100'S OF DOLLARS
1630 :             -RETURN TO INVESTOR--CONTRIBUTION TO SF.   'L
1635 : PRD    CASH  EARNINGS PRINCIPL  CASH TO EARN ON SF  INVEST.   SINK.FD'L
...
2620 :  YR   INCOME INTEREST OTH.EXP  LOSS/GN   TAXES    ITC  CF EARN NET EARN '
2625 PRINT
2630 FOR Y=1 TO S
...
2821 PRINT " DEBT TYPE SPECIFIED IS",A$
...
3045 REM D/S
3050 READ #1,I$
3055 READ #1,D1,Y2,Q2,Y3,Q3
3060 D2=W1*(Y2-1)+Q2
3065 D3=W1*(Y3-1)+Q3
3070 FOR Y=D2 TO D3
3075 M(Y,4)=D1
3080 NEXT Y
3085 IF D3>=Q1 GOTO 3095
3090 GOTO 3055
3095 B1=0
3100 REM "RENT STARTING YR,PRD;ENDING YR,PRD
3105 READ #1,I$
3110 READ #1, B1,Y2,Q2,Y3,Q3
3115 B2=W1*(Y2-1)+Q2
3120 B3=W1*(Y3-1)+Q3
3125 FOR Y=B2 TO B3
3130 M(Y,1)=B1
3135 NEXT Y
3140 IF B3>=Q1 GOTO 3150
3145 GOTO 3110
3150 REM "EXP";
3155 READ #1,I$
...
3261 INPUT W$
3262 M$="A"
3263 IF LEFT$(O$,1)="A" THEN 3271
3266 IF LEFT$(W$,1)="N" THEN 3290
3267 PRINT "ANNUAL OR ";O$;"LY";
3268 INPUT M$
3271 J$=O$
3272 O$="   "
3275 IF LEFT$(M$,1)="A" THEN 3290
3280 M$="P"
3281 O$=J$
...
3355 : YR 'LLLLL RENTAL  DEPRECIA-   INTEREST  INTEREST &   OTHER    INTERNAL   TAX LOSS   TAX SAVING AFTER TAX  CUMULATIVE '
...
3610 PRINT USING 3614,(Y+W1-1)/W1,L1,L2,L3,L4,L5,L6,L7,L8,L0,T5
3614:###    ########## ######### ######### ########## ########## ########## ########## ########## ########## ########### ###########
3615:### ### ######### ######### ######### ########## ########## ########## ########## ########## ########## ########### ###########
3620:'LLL    ######### ######### ######### ########## ########## ########## ########## ########## ########## ########### ###########
...
3665 PRINT
3670 PRINT
3675 PRINT USING 3620, "SALE",K0,0,0,K1,K1-X0,-K2,K3,T5+K3
3680 PRINT
3685 PRINT USING 3690,V1,V2,V3,V4,V5,V6,V7,V8,T5+K3,T5+K3
3690: TOT    ######### ######### ######### ########## ########## ########## ########## ########## ########## ########### ###########
3695 PRINT
3700 B1=Q1+1
...
4076 O$=J$
4077 U$="A"
4078 IF LEFT$(O$,1)="A" THEN 4086
4080 PRINT"ANNUAL OR ";O$;"LY";
4085 INPUT U$
4086 O$=U$
4090 U$=LEFT$(U$,1)
4091 IF U$="A" THEN 4095
4092 U$="P"
4093  GOTO 4096
4095 O$="   "
4096  PRINT
...
4227 PRINT USING 4014,0,N(Y,1),N(Y,2),N(Y,3),N(Y,4),N(Y,5),-N(Y,6),N(Y,7)
4226 GOTO 4230
...
4250 F2=F2+1
4255 PRINT USING 4014,F2,U1,U2,U3,U4,U5,-U6,U7
...
5130 :   YR     INCOME     INTEREST  OTHER EXP.   LOSS/GAIN    TAXES       ITC       C.F. EARN   NET EARNINGS '
5135 PRINT
5140 FOR Y=1 TO S
...
5180 GOTO 9999
5185 GOTO 9999
5900 REM CALCULATIONS FOR HALF AND MODIFIED HF-YR CONVENTIONS.
5910 IF D0=1 THEN 5990
5912 IF D0=2 THEN 7250
5990 REM HALF YEARLY CONVENTION----
5992 IF LEFT$(S$,1)="Y" THEN 7100
6000 REM  CORPORATION EXISTED ONE TAX YEAR. HALF YEAR'S DEPRECIATION ALLOWED
6005 T2=.5
6010 T3=1
6015 Q0=W1-Q3+1
6020 FOR X=1 TO Q0
6025 C=C+1
6030 M(C,2)=M(C,2)+H(2,1)*T2/Q0
6035 S4=S4+M(C,2)+S4
6040 NEXT X
6045 FOR X=T3 TO X2-1
6050 T1=(H(2,X)*T2+H(2,X+1)*(1-T2))/W1
6055 FOR Y=1TO W1
6060 C=C+1
6065 M(C,2)=M(C,2)+T1
6070 S4=S4+M(C,2)
...
6080 NEXT X
6090 T1=H(2,X2)/W1
6095 FOR Y=1 TO W1*T2
6100 C=C+1
6105 M(C,2)=M(C,2)+T1
6110 S4=S4+M(C,2)
6200 NEXT Y
7000 GOTO 8100
7100 REM HALF YEAR CONVENTION - NEW CORPORATION--- N1=PERIOD IN EXISTANCE
7105 N1=W1-N0+1
7106 N1=N1*.5
7110 T3=H(2,1)*(N1/W1)
7115 Q0=W1-Q3+1
7120 FOR X=1 TO Q0
7125 C=C+1
7130 M(C,2)=M(C,2)+T3/Q0
7135 S4=S4+M(C,2)
7140 NEXT X
7145 FOR X=1 TO X2-1
7150 T4=H(2,X)*(N1/W1)+H(2,X+1)*(1-N1/W1)
7155 FOR Y=1 TO W1
7160 C=C+1
7165 M(C,2)=M(C,2)+T4/W1
7170 S4=S4+M(C,2)
7175 NEXT Y
7180 NEXT X
7185 T4=H(2,X2)*(1-N1/W1)
7190 N2=INT(W1-N1+.5)
7195 FOR Y=1 TO N2
7205 M(C,2)=M(C,2)+T4/N2
7210 S4=S4+M(C,2)
7215 NEXT Y
7220 GOTO 8100
7250 REM MODIFIED HF-YR.
7255 IF LEFT$(S$,1)="Y" THEN 7500
7260 IF W1-Q3+1<=6 THEN 7400
7300 REM  MODIFIED HALF-YR CONVENTION.AASSETS BOUGHT DURING FIRST HALF YEAR.
7301 REM FULL YEAR DEPRECIATION ALLOWED.
7305 T2=W1-Q3+1
7310 FOR X=1 TO T2
7315 C=C+1
7320 M(C,2)=M(C,2)+H(2,1)/T2
7325 S4=S4+M(C,2)
7330 NEXT X
7335 T4=2
7340 FOR X=T4 TO X2
...
7375 NEXT X
7380 GOTO 8100
7400 REM MODIFIED HALF-YRLY CONVENTION. ASSTES BOUGHT DURING THE SECOND
7401 REM  HALF OF THE YEAR. NO DEPRECIATION IS ALLOWED DURING THE FIRST
7402 REM YEAR.
7405 C=C+W1-Q3
7410 T4=1
7415 GOSUB 7340
7420 RETURN
7430 GOTO 8100
7500 REM MODIFIED HALF-YEAR -- NEW CORPORATION--- ASSETS BOUGHT DURING
7501 REM THE FIRST HALF OF THE EXISTANCE OF THE CORPORATION.
7502 N1=W1-N0+1
7503 IF W1-Q3+1<N1/2 THEN 7700
7505 T2=N1/W1
7510 N3=W1-Q3+1
7515 T4=(H(2,1)*T2)/N3
7520 FOR X=1TO N3
...
7540 NEXT X
7545 FOR X=1TO X2-1
7550 T3=(H(2,X)*N1/W1+H(2,X+1)*(1-N1/W1))/W1
7555 FOR Y=1 TO W1
7560 C=C+1
7565 M(C,2)=M(C,2)+T3
7570 S4=S4+M(C,2)
7575 NEXT Y
7576 NEXT X
7580 T3=H(2,X2)*(1-N1/W1)/(W1-N1)
7585 FOR Y=1TO W1-N1
7590 C=C+1
7595 M(C,2)=M(C,2)+T3
7600 S4=S4+M(C,2)
7605 NEXT Y
7610 GOTO 8100
7700 REM MODIFIED H-YR. NEW CORPORATION- ASSETS BOUGHT DURIN THE SECOND HALF YEAR.
7701 REM NO DEPRECIATION ALLOWED DURING THE FIRST YEAR.
7705 C=N3
7710 T4=1
7715 GOSUB 7340
7720 GOTO 8100
