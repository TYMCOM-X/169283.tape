...
8012 PRINT "         WHOLE                         WHOLE AND YIELD";K7*100;"% RESI"
8013 PRINT "                                       DISCOUNTED BACK BY";K8*100;"%"
8015 PRINT "YR. /";O$
8016 PRINT
8020 FOR Y=1TOQ1
...
8102 K4=INT(K4+.5)
8104 K5=INT(K5+.5)
8105 T=C(Y,2)+K5-N(Y+1,6)-N(Y+1,7)+Q1*H7-Y*H7-K1
8106 C(Y,1)=INT(C(Y,1)+.5)
8107 C(Y,2)=INT(C(Y,2)+.5)
8108 N(Y+1,6)=INT(N(Y+1,6)+.5)
8109 N(Y+1,7)=INT(N(Y+1,7)+.5)
8110 X=(T-(C(Y,1)-K1)*Z1)/(1-Z1)
8111 IF FNM(Y,W2)<>0 THEN 8120
8112 REM PRINT INT(T);C(Y,1);C(Y,2),N(Y+1,6),N(Y+1,7);K4;INT(Q1*H7-H7*Y)
8115 PRINT USING 8130,INT((Y-1)/W1+1),FNA(Y),X,X+K4
8120 NEXT Y
8130 :###/###     ##########               #############
9000 GOTO 9992
9990 GOSUB 8000
