...
1180 READ I6,Y2,Q2,Y3,Q3
1200 B2=4*(Y2-1)+Q2
1220 B3=4*(Y3-1)+Q3
1240 FOR Y=B2 TO B3
1260 M(Y,3)=I6+M(Y,3)
1280 NEXT Y
1300 IF I6=0 THEN 1660
1320 GOTO 1180
...
1672 M(Y,4)=D1
1674 NEXT Y
1676 IF D3>=Q1 GOTO 1680
1678 GOTO 1664
1680 C=0
1700 FOR X=1 TO Q1/4
1720 H(2,X)=H(2,X)/4
1740 E1=E1/4
1760 FOR Y=1 TO 4
1780 C=C+1
...
2700 FILE #3,F$
2720 SCRATCH #3
3020 IF W$="NO" THEN 3220
3022 FOR I=1TO4
3024 PRINT
3026 NEXT I
3028 PRINT SPACE$(50);L$
...
3240 IF X3<>0 GOTO 3280
3260 X3=X1-S4
3280 FOR Y=1 TO Q1+1
3300 M(Q1+1,1)=K1
3320 L9=L9+1
3340 M(Y,2)=M(Y,2)+H7
3360 M(1,2)=M(1,2)+H6
...
3540 M(1,9)=M(1,9)+I5
3560 REM M(1,9)=M(1,9)+I5/4
3580 REM M(2,9)=M(2,9)+I5/4
3600 REM M(3,9)=M(3,9)+I5/4
3620 REM M(4,9)=M(4,9)+I5/4
3640 M(Q1+1,9)=K1-(K1-X3)*Z1
3660 L1=L1+M(Y,2)
3680 V1=V1+M(Y,1)
3700 V2=V2+M(Y,2)
3720 V3=V3+M(Y,3)
...
4020 IF M$="NO" THEN 4220
4040 A5=4
4060 IF L9<>4 THEN 4240
4080 IF M(Y,1)<>0 GOTO 4140
4100 PRINT USING 4160,Y/4,M(Q1-1,1)*3,L1,L2,M(Q1-1,4)*3,M(Y,5)*4,L3,L4,L5,L6,T5
4120 IF M(Y,1)=0 GOTO 4180
4140 PRINT USING 4160, Y/4,M(Y,1)*A5,L1,L2,M(Y,4)*4,M(Y,5)*4,L3,L4,L5,L6,T5
4160: ### ######### ######### ######### ########## ########## ########## ########## ########## ########## ########### ###########
4180 L1=L2=L3=L4=L5=L6=L7=L8=L9=0
4200 GOTO 4240
...
4300 K2=(K1-X3)*Z1
4320 K3=K1-((K1-X3)*Z1)
4340 IF M$="Q" THEN 4380
4360 PRINT USING 4160, (Q1/4)+1,K1,X3,0,0,0,K1,K1-X3,-K2,K3,T5
4380 PRINT
4400 PRINT USING 4420,V1,V2,V3,V4,V5,V6,V7,V8,V9,T5
4420: TOT ######### ######### ######### ########## ########## ########## ########## ########## ########## ########### ###########
4440 PRINT
4460 B1=Q1+1
...
4680 GOTO 4800
4700 PRINT "SF RATE,*P/Y,INVESTMENT,RESIDUAL";
4720 INPUT R,P,C,Z2
4740 Z3=Z2-((Z2-X3)*Z1)
4760 A(B1,1)=Z3
4780 GOTO 4860
...
4880 R5=R
4900 REM: ASSORT NEGATIVE FROM POSITIVE
4920 T=0
4940 FOR Y=1 TO B1
4960 T =T+1
4980 IF A(Y,1)<0 GO TO 5020
5000 NEXT Y
5020 FOR Y = T TO B1
5040 A (Y,2) = A (Y,1)
5080 C(Y)=0
5100 NEXT Y
5120 FOR Y =B1 TO T STEP -1
5140 C(Y)=C(Y)+A(Y,2)/((1+R/P)^Y)
5160 N5=N5+C(Y)
5180 NEXT Y
5200 FOR Y = T-1 TO 1 STEP -1
5220 N2=N1+N5
5240 N1 = N1+A(Y,1)/((1+R/P)^Y)
5260 IF N1+N5 <1 GOTO 5340
5280 Y1=Y
5300 N3=ABS(N2)*((1+R/P)^Y)
5320 GOTO 5360
5340 NEXT Y
5360 FOR Y=1 TOY1
5380 A(Y,4)=A(Y,1)
5400 NEXT Y
5420 A(Y1,4)=A(Y,1)-N3
5440 C1=P
5460 FOR I=1 TO B1
5480 DIM Z(121)
5500 Z(I)=A(I,4)
...
5680 Y=0
5700 FOR I=1 TO B1
5720 Y=Y+Z(I)/((1+R/C1)^I)
5740 NEXT I
5760 IF Y-C>0 THEN 5660
5780 R=R-Z
5800 NEXT J
5820 PRINT "AT RESIDUAL  ";A(B1,1),Z3
...
6000 FOR Y=2 TO B1+1
6020 IF N(Y-1,4)<0 GOTO6100
6040 N(Y,5)=N(Y-1,4)*(R5/4)
6060 N(Y,4)=N(Y-1,4)+N(Y,5)+N(Y,1)
6080 GOTO 6340
6100 IF N(Y,1)<=0 GOTO 6280
6120 N(Y,2)=ABS(N(Y-1,4))*(R/4)
6140 N(B1+1,2)=ABS(N(B1,4))*(R5/4)
6160 N(Y,3)=N(Y,1)-N(Y,2)
6180 N(Y,4)=N(Y-1,4)+N(Y,3)
6200 IF N(Y,2)>N(Y,1) THEN 6240
6220 GOTO 6340
6240 N(Y,3)=N(Y,1)-N(Y,2)
6260 GOTO 6320
6280 N(Y,2)=N(Y-1,4)*(R5/4)
6300 N(Y,3)=N(Y,1)+N(Y,2)
6320 N(Y,4)=N(Y-1,4)+N(Y,3)
6340 NEXT Y
6360 PRINT "PRINT";
6380 INPUT R$
6400 IF R$="NO" THEN 6980
6420 PRINT"1=QTRLY,2=YRLY";
6440 INPUT U$
6460 R6=R*100
6480 PRINT USING 6520,N$
6500 R7=R5*100/4
6520 :    PERIOD       CASH        INTEREST     PRINCIPAL    BALANCE      EARNINGS '
6540 PRINT USING 6560,R6,R7
6560 :              AVAILABLE      AT###.###%    PAYMENT    REMAINING     AT###.###%
6580 PRINT
6600 PRINT
6620 FOR Y=1 TO B1+1
...
6740 U4=N(Y,4)
6760 U5=U5+N(Y,5)
6780 IF U$="2" GOTO 6860
6800 PRINT USING 5940,F1,N(Y,1),N(Y,2),N(Y,3),N(Y,4),N(Y,5)
6820 F1=F1+1
6840 GOTO 6960
6860 IF Y=B1+1 GOTO 6900
6880 IF U<>4 GOTO 6960
6900 F2=F2+1
6920 PRINT USING 5940,F2,U1,U2,U3,U4,U5
6940 U=U1=U2=U3=U4=U5=0
6960 NEXT Y
6980 GOTO 8900
7000 REM DEP ROUTINE+-DDB;2 YRS    SYD REMAINDER
7020 REM "DEPR AMT, YRS";
7040 READ #1, X1,X2
...
8680 H(2,E)=K
8700 NEXT I
8720 H(2,E+2)=K(1,E)-X3
8740 GO TO 540
8760 REM DATA
8780 DATA 165289,141322,126446,111570,96694,81818,64256,27066,0,0,0,0,0,0,0
8800 FOR Y = 1 TO X2
8820 READ H(2,Y)
8840 NEXT Y
8860 GOTO 530
8880 REM EARNINGS
8900 PRINT "UNEARNED INC";
8920 INPUT E(1)
8940 PRINT "QTLY CF EARNGS RATE";
8960 INPUT E9
8980 PRINT "ITC;1=PASS THRU, 2=AMORTIZED";
9000 INPUT E$
9020 IF E(1)=0 THEN 10300
9040 IF E$="1" GOTO 9100
9060 E(6) = I5
9080 GOTO 9100
9100 FOR P= 1 TO 6 STEP 5
9120 H=0
9140 S=Q1/4
9160 FOR I=1 TO S
9180 H=H+I
...
9660 C1=C2=0
9680 T6=0
9700 FOR Y=1 TO Q1+1
9720 C1=C1+1
9740 T6=T6+T(Y,1)
9760 B(Y,10)=T6
9780 B(Y,11)=B(Y,10)+B(Y,12)
9800 B(Y+1,12)=B(Y,11)*E9
9820 C2=C2+B(Y,12)
9840 IF C1<>4 THEN 9900
9860 B(Y/4,7)=C2
9880 C1=C2=0
9900 NEXT Y
9920 PRINT
9940 IF E$<>"1" GOTO 9980
9960 B(1,6)=I5
9980 PRINT USING 10000,N$
10000 :   YR     INCOME    INTEREST    INTEREST   LOSS/GAIN    TAXES       ITC       C.F. EARN   NET EARNINGS '
10060 PRINT
10080 FOR Y=1 TO S
10140 B(Y,4)=B(Y,1)+B(Y,14)-B(Y,2)-B(Y,3)
10160 B(Y,5)=B(Y,4)*.5
10180 B(Y,8)=B(Y,5)+B(Y,6)+B(Y,7)
10200 PRINT USING 10220,Y,B(Y,1),B(Y,2),B(Y,3),B(Y,4),B(Y,5),B(Y,6),B(Y,7),B(Y,8)
10220 :  ###  #######.##  #######.##  #######.##  #######.##  #######.## #######.##  #######.## #######.##
10240 NEXT Y
10260 GOTO 8900
10280 GOTO 10300
