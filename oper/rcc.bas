...
196 INPUT FROM 4:P,O1,O2,O3,I,F,N,N3,L,M,K,R,R3,R4,D,E,F2,Q,T,A1,
A2,I6,O5,X7,X8,C
198 Z8="^#DDDD,DDD,DDD.  ^*"
199 IF O2=1 THEN R=R3
200 I=I/100,E=E/100,T=T/100
203 IF A1=1 OR A1=3 THEN M=1.5
204 IF A1=2 OR A1=4 THEN M=2
207 IF A1=1 OR A1=2 OR A1=3 OR A1=4 THEN 213
211 A1=0
212 GOTO 215
213 IF A2>0 THEN IF A2<13 THEN 215
214 A2=1
215 GOTO 221
221 IF O2=0 THEN 279
222 IF O2=1 THEN 280 ELSE 9070
...
264 IF O3=2 THEN 460 ELSE 460
266 L=L/(1+I/F)
267 GOTO 460
279 N3=0
280 N5=INT(((N*12)+A2+11)/12)
...
330 IF C=0 THEN 390
332 IF C=1 THEN 340 ELSE 9010
340 FOR Y=1 TO N2
350 INPUT FROM 4:C(Y)
360 NEXT Y
370 GOTO 420
390 FOR Y= 1TO N2
400 C(Y)=0
...
472 Y8=Y9
473 Y=Y+1
474 IF (Y8+F)>Y7 THEN 480
475 Y8=Y8+F
476 L(Y)=L*F
477 GOTO 473
480 L(Y)=L*(Y7-Y8)
485 A=N5+1
505 IF O2= 0 THEN 520 ELSE 512
511 A=1
512 FOR Y=A TO N4
513 INPUT FROM 5:L(Y)
514 NEXT Y
...
574  IF O3=1 THEN Q1=Q2/F+Q1*W,Q2=Q2*W
578 U=U+Q2
580 U1=U1+Q1
582 NEXT Y
...
590 IF O3=1THEN Q1=Q2/F+Q1*W,Q2=Q2*W
594 U=U+Q2
596 U1=U1+Q1
598 I=I-(U-P)/U1
...
610 IF R4-P/10>0 THEN 630 ELSE 640
630 W=R4-P/10
640 Z=(P-W)/K
650 B=P
660 IF A1>0 THEN IF A1<5 THEN 957
670 IF M=1 THEN 720
675 IF K<3 THEN PRINT "1.5DB AND DDB NOT ALLOWED, EQUIP LIFE <3YR"
676 IF K<3 THEN 9999
680 IF M=1.5 THEN 830
690 IF M=2 THEN 830 ELSE 9030
...
790 IF K>Y THEN D(Y+1)=Z*(K-Y),Y=Y+1
810 GOTO 1010
...
971 D1=B/K3
972 IF K<3 THEN 994
974 IF A3=1 THEN 994
978 H=INT(K2)
980 G=(H+2*(K2-H))*(H+1)/2
982 D2=(B*K2)/G
984 D3=(B*M)/K
988 IF D3>D2 THEN D4=D3 ELSE D4=D2
...
1170 W=(1+E/F2)^(Q*F2)
1180  X=D*E/F2*W/(W-1)
1190 Y=0,D1=D,F8=A2,F9=12/F2,F7=INT(12*Q)
1205 F6,P(N2+1),E(N2+1)=0
1230 Y=Y+1
1235 IF F6>=F7 THEN 1370
1240 P(Y)=0
1250 E(Y)=0
1270 P1=X-(D1*E/F2)
1280 P(Y)=P(Y)+P1
...
1310 F8=F8+F9
1320 F6=F6+F9
1325 IF F6>=F7 THEN 1350
1330 IF F8<=12 THEN 1270
1340 F8=F8-12
...
1500  T(N2+1)=0
1501 IF O5=0 THEN 1519
1502 DIM G(51),H(51)
1503 FOR Y= 1 TO O5
1504 INPUT FROM 6:L8,L9
1505 G(Y)=L8
1506 H(Y)=L9
...
1518 DIM I(51)
1519 FOR Y=1 TO N5
1520 T(Y)=(L(Y)-D(Y)-E(Y)-C(Y)-I(Y))*T
1530  T(N2+1)=T(Y)+T(N2+1)
1540  NEXT Y
1541 Z=0
...
1546 T(Y)=T*(L(Y+Z)-D(Y)-E(Y)-C(Y)-I(Y))
1547 T(N2+1)=T(Y)+T(N2+1)
1548 NEXT Y
1550  REM COMPUTE AFTER TAX CASH FLOW F(Y)
1560  F(N2+1)=0
1570 FOR Y=1 TO N5
1580 F(Y)=L(Y)-E(Y)-P(Y)-T(Y)-C(Y)-I(Y)
1590  F(N2+1)=F(Y)+F(N2+1)
1591 NEXT Y
1592 IF Z=1 THEN F(Y)=F(Y)+L(Y+1),F(N2+1)=F(N2+1)+L(Y+1)
1594 FOR Y=N5+1 TO N2
1595 F(Y)=L(Y+Z)-E(Y)-P(Y)-T(Y)-C(Y)-I(Y)
1596  F(N2+1)=F(N2+1)+F(Y)
1600  NEXT Y
1601 IF X7=0 THEN 1605
...
1660 V=I6,W=1,X,U,Z,J=0
1675 A=1+V/F,B=1+V/F2,C=1+V/4
1678 GOTO 1640
1680 P1=P*A^-G9
1681 Y1,Y2=1,M1=A2,F9=G6
1685 PRINT P1
1686 P1=FNE(A,A2,G7,P1)
1687 PRINT A,A2,G7,P1
1690 IF F9=0 THEN 1694
1691 X2=L(Y1)-C(Y1)
1692 Z=Z+FND(X2,F9,A,G9,Y2)
1694 IF Y2>=G8 THEN 1712
1695 Y2=Y2+1
1697 M1=M1+G7
1699 IF M1<=12 THEN 1690
1701 M1=M1-12
1702 Y1=Y1+1
1704 F9=F
1706 IF INT((G8+G9)/F)>(Y1-1) THEN 1690
1708 F9=G5
1710 GOTO 1690
...
1739 X=X+J
1740 IF Y2>=G8 THEN 1761
1741 M1=M1+G7
1742 Y2=Y2+1
1743 IF M1<=12 THEN 1736
1744 M1=M1-12
1746 Y1=Y1+1
1748 F9=4
1750 IF INT((G8+G9)/4)>(Y1-1) THEN 1736
1752 F9=G5
1754 GOTO 1736
1757 IF F9=0 THEN 1760
1758 X2=T*(L(Y1)-C(Y1)-I(Y1)-D(Y1)-E(Y1))
1759 Z=Z-FND(X2,F9,C,G9,Y2)
1760 RETURN
1761 IF D=0 THEN 1818
...
1786 F9=G6
1788 GO SUB 1810
1790 IF Y2>=G8 THEN 1816
1792 Y2=Y2+1
1794 M1=M1+G7
1796 IF M1<=12 THEN 1788
1798 M1=M1-12
1800 Y1=Y1+1
1802 F9=F2
1804 IF INT((G8+G9)/F2)>(Y1-1) THEN 1788
1806 F9=F5
1808 GOTO 1788
1810 X2=P(Y1)+E(Y1)
1811 IF F9=0 THEN 1814
...
1821 L6=0
1822 IF O5=0 THEN 1826
1823 FOR Y=1 TO O5
1824 L6=L6+G(Y)*A^(-H(Y))
1825 NEXT Y
1826 U=U+X7*A^(-X8)-L6
1828 REM PRINT W,V,U TO WATCH PROGRESS OF LOOP
1830 IF W=1 THEN 1850 ELSE 1920
1850 U1=U,U2=V
1870 W=2,V=1.001*V
1890 GOTO 1671
...
5040 PRINT IN FORM "PURCHASE VALUE OF EQUIPMENT $999,999,999":P
5050  PRINT "PRETAX RATE: ":I*100:"%":B$
5060 PRINT IN FORM "RATE OF RETURN: 999.9999 %":V*100
5070 PRINT "ORIGINAL LEASE ":F*N:" PERIODS"
5075 IF O2=1 THEN PRINT "EXTENSION ":F*N3:" PERIODS"
5080 PRINT IN IMAGE "EQUIPMENT LIFE %%.%% YEARS  LEASE STARTS IN
MONTH %%%":K,A2
5085 IF D=0 THEN 5105
5090 PRINT "DEBT LIFE ":Q:" YEARS"
5105 IF D=0 THEN 5120
5110 PRINT "DEBT INTEREST ":E*100:"%"
5120 PRINT "TAX RATE ":T*100:"%"
5121 IF X7=0 THEN 5130
5122 PRINT IN FORM "ITC $999,999,999  IN PERIOD 999":X7,X8
...
TAB(51):"FEDERAL":TAB(63):"AFTER TAX"
5190PRINT "YR";TAB(6):"PAYMENT";TAB(17):"DEPREC'N";TAB(28):"INTEREST";
"PRINCIPAL":TAB(53):"TAX":TAB(63):"CASH FLOW"
5210 FOR Y=1 TO N5
5220 Z8="%% 2B 9%. 1B 9%. 1B 9%. 1B 9%. 1B 9%. 1B 9%."
5230 PRINT IN FORM Z8:Y,L(Y),D(Y),E(Y),P(Y),T(Y),F(Y)
5250 NEXT Y
5251 IF N>INT(N) THEN X=1
5252 FOR Y=N5+1 TO N2
5253 PRINT IN FORM Z8:Y,L(Y+X),D(Y),E(Y),P(Y),T(Y),F(Y)
5254 NEXT Y
5260 Y=0
5270 PRINT IN FORM Z8:N1,Y,Y,Y,Y,U4,R-U4
5280 PRINT "    ---------- ---------- ---------- --------- ---------
 ---------"
5300 Y= N2 +1
5310 Z7="4B 9%. 1B 9%. 1B 9%. 1B 9%. 1B 9%. 1B 9%."
5320 PRINT IN FORM Z7:L(Y+1),D(Y),E(Y),P(Y),T(Y)-X7,F(Y)+X7
5340 PRINT
5350 PRINT
5360 PRINT IN IMAGE " RESIDUAL VALUE OF EQUIPMENT = $%%%,%%%,%%%.":R
5380 PRINT
5390 PRINT IN IMAGE " BOOK VALUE OF EQUIPMENT = $%%%,%%%,%%%.":P-D(N2+1)
5410 PRINT
5420 PRINT
5422 LINK "@RCCB"
...
5436 PRINT
5437 PRINT
5440 PRINT TAB(12):"------EQUITY ANALYSIS------";TAB(48):"TERMINATION"
:TAB(64):"MISC."
5460 PRINT "YEAR";TAB(X):"RETURN";TAB(21):"RECOVERY";TAB(35):"OUTSTD'G":
TAB(51):"VALUE":TAB(63):"EXPENSE"
5480 PRINT
5490 C2,V2,P2,Z,S2=0
5530 B1=P-D
5550 FOR Y=1 TO N
5555 GO SUB 5560
5557 NEXT Y
...
5630 C2=C2+C(Y)
5640 Z6=" DD DDDD,DDD,DDD. DDDD,DDD,DDD. DDDD,DDD,DDD. DDDD,DDD,DDD.
 DDD,DDD,DDD."
5645 PRINT IN IMAGE Z6:Y,R1,S1,B1,V1,C(Y)
5650 RETURN
...
5673 GO SUB 5580
5674 GO TO 5720
5675 PRINT IN IMAGE Z6:N1,0,S1,B1-S1,TAB(59):0
5676 S2=S2+S1
5680 PRINT TAB(5):"------------  ------------  ------------  -----
-------  -----------"
5700 PRINT IN FORM "%% 1B DDDD,DDD,DDD.29B DDD,DDD,DDD.":17,S2,C2
5715 GO TO 5999
...
5765 GO TO 5675
5999 F(N1)=F(N1)+R-U4
6000 PRINT "ACCOUNTING?  ":
6001 T(N1)=T(N1)+U4
6010 INPUT W
6020 IF W="NO" THEN 9999
6030 IF W#"YES" THEN 6000
6031 DIM W(51),X(51),Z(51)
6032 IF X7=0 THEN 6041
6033 Z9=X7/(N+N3)
6034 FOR Y=1 TO N2
6035 Z(Y)=Z9
6036 NEXT Y
6040 Z(Y)=X7-Z9*(Z-1)
6041 IF O2=0 OR N=INT(N) THEN 6050
...
7481 FOR Y=Z-W TO Z
7482 PRINT IN IMAGE Z8:15,T*X(Y)-Z(Y)
7483 NEXT Y
7485 PRINT
7486 IF W<3 THEN PRINT
7490 PRINT "  NET AFTER TAX";
7500 FOR Y=Z-W TO Z
7510 PRINT IN IMAGE Z8:15,(1-T)*X(Y)+Z(Y)
7520 NEXT Y
7525 GO TO 7580
...
7590 PRINT
7600 PRINT
7602 IF Y<5 THEN PRINT, "BALANCE SHEET" ELSE PRINT, "BALANCE SHEET CONTINUED"
7606 PRINT
7610 PRINT "ASSETS"
7620 PRINT "  CASH";
7625 X4=C1
...
7690 PRINT IN IMAGE Z8: 15, L(N2+2)+R-X4
7700 NEXT Y
7701 C2=X4
...
7720 FOR Y=Z-W TO Z
7730 X4 = X4+W(Y)
7740 PRINT IN IMAGE Z8: 15, X4+P-L(N2+2)-R
7750 NEXT Y
7751 C3=X4
7760 GO SUB 7252
...
7800 PRINT IN IMAGE Z8: 15, X4+P
7810 NEXT Y
7811 C4=X4
7820 GO SUB 7530
7822 IF W<3 THEN PRINT
7825 PRINT
7830 PRINT
...
7860 FOR Y=Z-W TO Z
7870 X4=X4+P(Y)
7880 PRINT IN IMAGE Z8: 15, D-X4
7890 NEXT Y
7891 C5=X4
...
8030 PRINT IN IMAGE Z8: 15, D+X4
8040 NEXT Y
8041 C7=X4
8050 GO SUB 7530
8060 IF HEN PRINT
...
8270 PRINT IN IMAGE Z8: 15, C9+P-D
8280 NEXT Y
...
8360 PRINT IN IMAGE Z8: 15, C0+P
8370 NEXT Y
8375 GO SUB 7530
